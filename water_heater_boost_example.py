...
from gehomesdk.erd.values.water_heater import ErdWaterHeaterBoostState, ErdWaterHeaterBoostControl
...
async def log_state_change(data: Tuple[GeAppliance, Dict[ErdCodeType, Any]]):
    ...
    if appliance.appliance_type == ErdApplianceType.WATER_HEATER:
        ...
        if ErdCode.WH_HEATER_BOOST_STATE in state_changes:
            boost_mode = state_changes[ErdCode.WH_HEATER_BOOST_STATE]
...
async def detect_water_heater(appliance: GeAppliance):
    ...
    if appliance.appliance_type == ErdApplianceType.WATER_HEATER:
        ...
        try:
            boost_mode = appliance.get_erd_value(ErdCode.WH_HEATER_BOOST_STATE)
...
async def toggle_boost_mode(client, enable: bool):
    ...
    boost_mode = ErdWaterHeaterBoostControl.ON if enable else ErdWaterHeaterBoostControl.OFF
    ...
    for heater in water_heaters:
        ...
        try:
            ...
            # WH_HEATER_BOOST_STATE (0x4220) is read-only
            ...
            try:
                current_boost_mode = heater.get_erd_value(ErdCode.WH_HEATER_BOOST_STATE)
                ...
                updated_boost_mode = heater.get_erd_value(ErdCode.WH_HEATER_BOOST_STATE)
